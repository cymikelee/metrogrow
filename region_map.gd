...
func _update_map_cell(tile_coords):
	var tile_N = get_cell_source_id(get_neighbor_cell(tile_coords, TileSet.CELL_NEIGHBOR_TOP_SIDE))
	var tile_S = get_cell_source_id(get_neighbor_cell(tile_coords, TileSet.CELL_NEIGHBOR_BOTTOM_SIDE))
	var tile_E = get_cell_source_id(get_neighbor_cell(tile_coords, TileSet.CELL_NEIGHBOR_RIGHT_SIDE))
	var tile_W = get_cell_source_id(get_neighbor_cell(tile_coords, TileSet.CELL_NEIGHBOR_LEFT_SIDE))
	...
	if(tile_N >= 0 && tile_S >= 0):
		...
		set_cell(tile_coords, 0, Vector2i(3, 0)) # blank
...
func _physics_process(delta) -> void:
	if(Input.is_action_just_pressed("mb_left")):
		...
		if(get_cell_source_id(coords_N) >= 0):
			_update_map_cell(coords_N)
		if(get_cell_source_id(coords_S) >= 0):
			_update_map_cell(coords_S)
		if(get_cell_source_id(coords_E) >= 0):
			_update_map_cell(coords_E)
		if(get_cell_source_id(coords_W) >= 0):
